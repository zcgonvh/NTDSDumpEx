...
struct regf_header {
  ...
  unsigned char timestamp[8];  /* 0x0000000C	Q-Word	last modify date in WinNT date-format */
  ...
  unsigned char name[0x1fc - 0x2c];   /* Seems like the hive's name is buried here, max len unknown */
...
struct  hbin_page {
  ...
  unsigned char data[1];     /* 0x0020   First data block starts here           */
...
unsigned short id;          /* 0x0000	Word	ID: ASCII-"sk" = 0x6B73        */
unsigned short dummy1;      /* 0x0002	Word	Unused                         */
  ...
  unsigned char  data[4];     /* Security data up to len_sk bytes               */
